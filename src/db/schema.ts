...
export const profiles = pgTable(
  ...
  (table) => [
    foreignKey({
      columns: [table.id],
      foreignColumns: [appAdmins.userId],
...
export const adminAuditLog = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      name: "admin_audit_log_scene_id_fkey",
    }).onDelete("set null"),
    index("admin_audit_log_created_idx").on(table.createdAt),
    index("admin_audit_log_actor_created_idx").on(table.actorId, table.createdAt),
    check("admin_audit_log_action_check", sql`action ~ '^[a-z][a-z0-9_.-]{1,79}$'::text`),
    check("admin_audit_log_affected_count_check", sql`affected_count >= 0`),
  ],
);
...
export const adminRateLimits = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.actorId, table.bucket, table.windowStartedAt] }),
    foreignKey({
      columns: [table.actorId],
      foreignColumns: [appAdmins.userId],
      name: "admin_rate_limits_actor_id_fkey",
    }).onDelete("cascade"),
    index("admin_rate_limits_window_idx").on(table.windowStartedAt),
    check("admin_rate_limits_request_count_check", sql`request_count > 0`),
  ],
);

export const adminUploadReservations = pgTable(
  ...
  (table) => [
    foreignKey({
      columns: [table.actorId],
      foreignColumns: [appAdmins.userId],
      name: "admin_upload_reservations_actor_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.sceneId],
      foreignColumns: [scenes.id],
      name: "admin_upload_reservations_scene_id_fkey",
    }).onDelete("cascade"),
    unique("admin_upload_reservations_object_path_key").on(table.objectPath),
    index("admin_upload_reservations_expiry_idx")
      .on(table.expiresAt)
      .where(sql`finalized_at IS NULL AND discarded_at IS NULL`),
    index("admin_upload_reservations_actor_idx").on(table.actorId),
    check(
      "admin_upload_reservations_purpose_check",
      sql`purpose = ANY (ARRAY['ambience'::text, 'background'::text])`,
    ),
    check(
      "admin_upload_reservations_bucket_check",
      sql`bucket = ANY (ARRAY['ambience-audio'::text, 'scene-media'::text])`,
    ),
    check(
      "admin_upload_reservations_state_check",
      sql`finalized_at IS NULL OR discarded_at IS NULL`,
    ),
  ],
);

export const adminStorageCleanupQueue = pgTable(
  ...
  (table) => [
    unique("admin_storage_cleanup_queue_bucket_object_path_reason_key").on(
      table.bucket,
      table.objectPath,
      table.reason,
    ),
    index("admin_storage_cleanup_pending_idx")
      .on(table.availableAt, table.createdAt)
      .where(sql`completed_at IS NULL`),
    check(
      "admin_storage_cleanup_queue_bucket_check",
      sql`bucket = ANY (ARRAY['ambience-audio'::text, 'scene-media'::text])`,
    ),
    check(
      "admin_storage_cleanup_queue_reason_check",
      sql`reason = ANY (ARRAY['expired_upload'::text, 'discarded_upload'::text, 'replaced_object'::text])`,
    ),
    check("admin_storage_cleanup_queue_attempts_check", sql`attempts >= 0`),
  ],
);

export const privateSchema = pgSchema("private");

export const ambienceAssets = pgTable(
  "ambience_assets",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("ambience_assets_active_idx").using(
      "btree",
...
export const ambienceAssetSources = pgTable(
  ...
  (table) => [
    index("ambience_asset_sources_asset_id_idx").using(
      "btree",
      table.assetId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    check(
      "ambience_asset_sources_original_byte_size_check",
      sql`(original_byte_size IS NULL) OR (original_byte_size > 0)`,
    ),
    check(
      "ambience_asset_sources_original_duration_seconds_check",
      sql`(original_duration_seconds IS NULL) OR (original_duration_seconds > (0)::numeric)`,
    ),
    check(
      "ambience_asset_sources_selected_duration_seconds_check",
      sql`(selected_duration_seconds IS NULL) OR (selected_duration_seconds > (0)::numeric)`,
    ),
    check(
      "ambience_asset_sources_selected_start_seconds_check",
      sql`(selected_start_seconds IS NULL) OR (selected_start_seconds >= (0)::numeric)`,
    ),
    check("ambience_asset_sources_source_order_check", sql`source_order > 0`),
    check(
      "ambience_asset_sources_source_sha256_check",
      sql`source_sha256 ~ '^[A-F0-9]{64}$'::text`,
    ),
  ],
);

export const ambienceAssetProvenance = privateSchema.table(
  "ambience_asset_provenance",
  {
    assetSourceId: uuid("asset_source_id").primaryKey().notNull(),
    sourceSha256: text("source_sha256").notNull(),
    originalFilename: text("original_filename"),
    originalByteSize: bigint("original_byte_size", { mode: "number" }),
    originalDurationSeconds: numeric("original_duration_seconds"),
  ...
  (table) => [
    foreignKey({
      columns: [table.assetSourceId],
      foreignColumns: [ambienceAssetSources.id],
      name: "ambience_asset_provenance_asset_source_id_fkey",
    }).onDelete("cascade"),
    check(
      "ambience_asset_provenance_source_sha256_check",
      sql`source_sha256 ~ '^[A-F0-9]{64}$'::text`,
    ),
    check(
      "ambience_asset_provenance_original_byte_size_check",
      sql`original_byte_size IS NULL OR original_byte_size > 0`,
    ),
    check(
      "ambience_asset_provenance_original_duration_seconds_check",
      sql`original_duration_seconds IS NULL OR original_duration_seconds > 0`,
    ),
    check(
      "ambience_asset_provenance_selected_start_seconds_check",
      sql`selected_start_seconds IS NULL OR selected_start_seconds >= 0`,
    ),
    check(
      "ambience_asset_provenance_selected_duration_seconds_check",
      sql`selected_duration_seconds IS NULL OR selected_duration_seconds > 0`,
    ),
  ],
);
...
export const scenes = pgTable(
  ...
  (table) => [
    ...
    check(
      "scenes_background_storage_path_safe",
      sql`(background_storage_path IS NULL) OR (background_storage_path ~ '^rooms/[a-z0-9-]+/background/[0-9a-f-]+\.webp$'::text)`,
    ),
    check(
      "scenes_foreground_text_color_hex",
      sql`foreground_text_color ~ '^#[0-9A-Fa-f]{6}$'::text`,
    ),
  ],
);

export const ambienceProfiles = pgTable(
  "ambience_profiles",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    sceneId: uuid("scene_id").notNull(),
  ...
  (table) => [
    index("ambience_profiles_scene_id_idx").using(
      "btree",
      table.sceneId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.sceneId],
      foreignColumns: [scenes.id],
    ...
    check(
      "ambience_profiles_fade_out_ms_check",
      sql`(fade_out_ms >= 0) AND (fade_out_ms <= 10000)`,
    ),
    check(
      "ambience_profiles_max_master_gain_check",
      sql`(max_master_gain >= (0)::numeric) AND (max_master_gain <= (1)::numeric)`,
    ),
    check(
      "ambience_profiles_music_duck_ratio_check",
      sql`(music_duck_ratio >= (0)::numeric) AND (music_duck_ratio <= (1)::numeric)`,
    ),
  ],
);

export const curatedSets = pgTable(
  "curated_sets",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    sceneId: uuid("scene_id").notNull(),
  ...
  (table) => [
    uniqueIndex("curated_sets_one_active_per_scene")
      .using("btree", table.sceneId.asc().nullsLast().op("uuid_ops"))
      .where(sql`is_active`),
    index("curated_sets_scene_order_idx").using(
      "btree",
      table.sceneId.asc().nullsLast().op("uuid_ops"),
...
export const curatedSetTracks = pgTable(
  "curated_set_tracks",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("curated_set_tracks_track_idx").using(
      "btree",
      table.trackId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const tracks = pgTable(
  ...
  (table) => [
    uniqueIndex("tracks_catalogue_key_key").using(
      "btree",
      table.catalogueKey.asc().nullsLast().op("text_ops"),
    ),
...
export const oneliners = pgTable(
  "oneliners",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    sceneId: uuid("scene_id").notNull(),
...
export const playbackSources = pgTable(
  "playback_sources",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    trackId: uuid("track_id").notNull(),
  ...
  (table) => [
    index("playback_sources_track_priority_idx").using(
      "btree",
      table.trackId.asc().nullsLast().op("uuid_ops"),
      table.isActive.asc().nullsLast().op("bool_ops"),
    ...
    unique("playback_sources_provider_provider_item_id_key").on(
      table.provider,
...
export const roomVisits = pgTable(
  ...
  (table) => [
    index("room_visits_scene_started_idx").using(
      "btree",
      table.sceneId.asc().nullsLast().op("uuid_ops"),
      table.startedAt.desc().nullsFirst().op("timestamptz_ops"),
    ),
    index("room_visits_started_idx").using(
      "btree",
      table.startedAt.desc().nullsFirst().op("timestamptz_ops"),
    ),
    foreignKey({
    ...
    check(
      "room_visits_last_heartbeat_check",
      sql`(last_heartbeat_at IS NULL) OR (last_heartbeat_at >= started_at)`,
    ),
  ],
);

export const playbackSourceFailures = pgTable(
  "playback_source_failures",
  {
    sourceId: uuid("source_id").notNull(),
    errorCode: integer("error_code").notNull(),
    failedOn: date("failed_on", { mode: "string" })
  ...
  (table) => [
    ...
    check(
      "playback_source_failures_error_code_check",
      sql`error_code = ANY (ARRAY[2, 5, 100, 101, 150, 153])`,
    ),
...
export const soundStems = pgTable(
  "sound_stems",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    sceneId: uuid("scene_id"),
    ...
    loopStartSeconds: numeric("loop_start_seconds", { precision: 8, scale: 3 })
      .default("0")
      .notNull(),
    loopEndSeconds: numeric("loop_end_seconds", { precision: 8, scale: 3 }),
  },
  (table) => [
    index("sound_stems_active_scene_idx").using(
      "btree",
      table.sceneId.asc().nullsLast().op("uuid_ops"),
      table.isActive.asc().nullsLast().op("bool_ops"),
      table.sortOrder.asc().nullsLast().op("int4_ops"),
    ),
    index("sound_stems_asset_id_idx").using(
      "btree",
      table.assetId.asc().nullsLast().op("uuid_ops"),
    ),
    index("sound_stems_scene_id_idx").using(
      "btree",
      table.sceneId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    check(
      "sound_stems_check",
      sql`(event_max_seconds IS NULL) OR (event_max_seconds >= event_min_seconds)`,
    ),
    check(
      "sound_stems_check1",
      sql`(loop_end_seconds IS NULL) OR (loop_end_seconds > loop_start_seconds)`,
    ),
    check("sound_stems_crossfade_ms_check", sql`(crossfade_ms >= 0) AND (crossfade_ms <= 10000)`),
    check(
      "sound_stems_event_min_seconds_check",
      sql`(event_min_seconds IS NULL) OR (event_min_seconds >= 5)`,
    ),
    check(
      "sound_stems_max_gain_check",
      sql`(max_gain >= (0)::numeric) AND (max_gain <= (1)::numeric)`,
    ),
    check(
      "sound_stems_min_gain_check",
      sql`(min_gain >= (0)::numeric) AND (min_gain <= (1)::numeric)`,
    ),
    check(
      "sound_stems_role_check",
      sql`role = ANY (ARRAY['base'::text, 'texture'::text, 'event'::text])`,
    ),
  ],
);
