...
import { getCleanupReferenceStatus } from "@/lib/admin-cleanup";
...
async function cleanup(request: Request) {
  ...
  for (const item of (queued ?? []) as Array<{
    ...
    const referenceStatus = await getCleanupReferenceStatus(supabaseAdmin, {
      bucket: item.bucket,
    ...
    if (referenceStatus === "referenced") {
      ...
      await supabaseAdmin
        .from("admin_storage_cleanup_queue")
        .update({ completed_at: new Date().toISOString(), last_error: "skipped_referenced" })
        .eq("id", item.id);
      continue;
    }
    if (referenceStatus === "unavailable") {
      failed += 1;
      ...
      await supabaseAdmin
        ...
        .update({
          ...
          last_error: "reference_check_unavailable",
        })
        .eq("id", item.id);
      continue;
    }
