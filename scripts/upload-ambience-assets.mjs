...
if (!assetArgument)
  throw new Error("Usage: bun scripts/upload-ambience-assets.mjs <prepared-asset-dir>");
...
const manifest = JSON.parse(await readFile(join(assetDir, "ambience-assets.json"), "utf8"));
const expectedMimeType = manifest.format.mimeType;
...
for (const asset of manifest.assets) {
  ...
  const { error } = await client.storage.from("ambience-audio").upload(asset.storagePath, data, {
    contentType: expectedMimeType,
    cacheControl: "31536000",
    upsert: true,
  });
  if (error) throw new Error(`${asset.storagePath}: ${error.message}`);
  const publicUrl = client.storage.from("ambience-audio").getPublicUrl(asset.storagePath)
    .data.publicUrl;
